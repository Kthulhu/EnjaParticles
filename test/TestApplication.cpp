    ...
    void TestApplication::KeyboardCallback(unsigned char key, int x, int y)
    {
        ...
        switch (key)
        {
            ...
            case 'g':
            {
                ...
                float4 minDomain = float4(2.5f, 2.5f, 2.5f, 1.0f);
                float4 maxDomain = float4(7.5f, 7.5f, 7.5f, 1.0f);
                float4 col1 = float4(0.05f, 0.15f, .8f, 0.1f);
                systems["water"]->addBox(nn, minDomain, maxDomain, false,col1);
                //ps2->system->addBox(nn, min, max, false);
                return;
            }
            case 'p': //print timers
