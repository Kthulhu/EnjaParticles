...
void zeroPoint(PointData* pt)
{
    ...
    pt[0].linear_force = (float4)(0.,0.,0.,0.);
    pt[0].color_lapl = 0.;
                           ...
                           )
{
    ...
    if (startIndex != 0xffffffff)
    {
        ...
        for (uint index_j=startIndex; index_j < endIndex; index_j++)
        {
            ...
            ForNeighbor(ARGV, pt, index_i, index_j, position_i, gp, prbp DEBUG_ARGV);
#endif
        }
    }
}
    ...
    for (int z=cell.z-1; z<=cell.z+1; ++z)
    {
        for (int y=cell.y-1; y<=cell.y+1; ++y)
        {
            for (int x=cell.x-1; x<=cell.x+1; ++x)
            {
                ...
                IterateParticlesInCell(ARGV, pt, num, ipos, index_i, position_i, cell_indices_start, cell_indices_end, gp,/* fp,*/ prbp DEBUG_ARGV);
