                       ...
                       )
{
    ...
    if (rlen <= 2.*prbp->smoothing_distance)
    {
        ...
        float4 springForce = -prbp->static_stiffness*(2.*prbp->smoothing_distance-rlen)*(r/rlen);

        float4 veli = vel[index_i];

        float ln_res =log(prbp->restitution_coef);

        float dampening = -2.*ln_res*(sqrt((prbp->static_stiffness*(massnorm))/((ln_res*ln_res)+(M_PI_F*M_PI_F))));
