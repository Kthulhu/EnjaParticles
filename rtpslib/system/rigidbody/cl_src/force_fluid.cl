                       ...
                       )
{
    ...
    float4 position_j = pos_j[index_j] * prbp[0].simulation_scale; 
    float4 r = (position_j - position_i); 
    r.w = 0.f; // I stored density in 4th component
    // |r|
    float rlen = length(r);
    // is this particle within cutoff?
    if (rlen <= 2.*prbp[0].smoothing_distance)
    {
        ...
        if(length(tangVel)>prbp[0].friction_static_threshold)
            frictionalForce = -prbp[0].friction_dynamic*sqrt(normalDot)*(normalize(tangVel));
        else
            frictionalForce = -prbp[0].friction_static*tangVel;
        */
        pt[0].linear_force +=normalForce; //(normalForce+frictionalForce);

        //pt[0].linear_force += r;//debug
    }
}
