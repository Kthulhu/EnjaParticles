									  ...
									  __local uint* sVMem)
{
	...
	uint4 value;
	value = valuesIn[globalId]; // not sure if needed
...
// GPUs than it is on compute version 1.2 GPUs.
//----------------------------------------------------------------------------
__kernel void reorderDataKeysValues(__global uint   *outKeys, 
                                    ...
                                    uint numElements,
                                    uint totalBlocks,
                                    __local uint2* sKeys2, // Enough shared memory on card?
                                    __local uint2* sValues2) // Do I need shared for values?
{
    __local uint sOffsets[16];
    __local uint sBlockOffsets[16];
