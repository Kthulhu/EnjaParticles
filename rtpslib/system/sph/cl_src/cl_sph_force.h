...
float Pi = sphp[0].K*(di.x - rest_density);
float Pj = sphp[0].K*(dj.x - rest_density);

float kern = -.5 * dWijdr * (Pi + Pj) * sphp[0].wspiky_d_coef;
//float kern = dWijdr * (Pi * idi * idi + Pj * idj * idj) * sphp[0].wspiky_d_coef;
float4 force = kern*r; 
...
float4 xsph = (2.f * sphp[0].mass * Wijpol6 * (velj-veli)/(di.x+dj.x));
pt[0].xsph += xsph * (float)iej;
pt[0].xsph.w = 0.f;
#endif

pt[0].force += force * (float)iej;
