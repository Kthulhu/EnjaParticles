...
namespace rtps
{
    void SPH::calculate()
    {
        ...
        */
        ...
        float mass = (0.0256/(int)(log((float)settings->GetSettingAs<unsigned int>("max_num_particles"))/log(2.0f)));         //Particle Mass [ kg ]
        ...
        float domain_vol = ((dmax.x - dmin.x) * (dmax.y - dmin.y) * (dmax.z - dmin.z))/6;
    ...
    void SPH::updateParams()
    {
		...
		sphp.rest_density=settings->GetSettingAs<float>("rest_density");
